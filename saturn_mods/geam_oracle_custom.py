...
from rdkit.Chem import Mol, QED, AllChem
...
class DockingVina(object):
    def __init__(self, target):
        ...
        self.vina_program = '/home/aih/serra.korkmaz/projects/saturn/oracles/docking/docking_grids/qvina02'
        self.receptor_file = f'/home/aih/serra.korkmaz/projects/saturn/oracles/docking/docking_grids/{target}.pdbqt'
...
class GEAMOracle(OracleComponent):
    ...
    GEAM's Oracle that combines 3 individual Oracle Components:
        1. Gradient Boosting Model for Docking
        2. QED
        3. SA Score
    """
    def __init__(self, parameters: OracleComponentParameters):
        ...
        model_path = "/home/aih/serra.korkmaz/projects/saturn/SurogateModel/Gradient_boosting/ckpt_chembl/gradient_boosting_regressor_braf.pkl"
    ...
    def gradient_boosting_oracle(self, smiles_list: np.ndarray[str]) -> Tuple[np.ndarray[float], np.ndarray[float]]:
        """
        Predict docking scores using the Gradient Boosting model.
        Args:
            smiles_list: List of SMILES strings.
        Returns:
            raw_docking_scores: Predicted raw docking scores.
            rewards: Normalized rewards (non-negative).
        """
        fingerprints = []
        for smiles in smiles_list:
            mol = Chem.MolFromSmiles(smiles)
            if mol:
                fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius=2, nBits=1024)  # Correctly generate 1024-bit vector
                fingerprints.append(np.array(fp))  # Convert to NumPy array
            else:
                fingerprints.append(np.zeros(1024))  # Fallback for invalid molecules

        fingerprints = np.array(fingerprints)  # Ensure fingerprints is a NumPy array with correct shape

        # Predict docking scores
        raw_docking_scores = self.gb_model.predict(fingerprints)
        rewards = np.clip(raw_docking_scores, 0, None)  # Ensure rewards are non-negative
        return raw_docking_scores, rewards
    
    def chemprop_oracle(self, smiles_list: np.ndarray[str]) -> Tuple[np.ndarray[float], np.ndarray[float]]:
        """
        Predict docking scores using the Chemprop model.
        Args:
            smiles_list: List of SMILES strings.
        Returns:
            raw_docking_scores: Predicted raw docking scores.
            rewards: Normalized rewards (non-negative).
        """
        checkpoint_path = "/home/aih/serra.korkmaz/projects/saturn/SurogateModel/Chemprop_model_regression/checkpoints_chembl/epoch=19-step=460_braf.ckpt"
        
        # Load the Chemprop model
        mpnn = models.MPNN.load_from_checkpoint(checkpoint_path)

        # Prepare Chemprop dataset
        ...
        with torch.inference_mode():
            ...
            raw_docking_scores = trainer.predict(mpnn, test_loader)

        # Combine predictions and flatten the array
        raw_docking_scores = np.concatenate(raw_docking_scores, axis=0).ravel()
    ...
    def _compute_property(
        ...
        """
        raw_docking_scores, docking_rewards = reward_vina(smiles, self.vina_oracle) #here adjust the oracle to use the chemprop model self.chemprop_oracle or gradient boosting model self.gradient_boosting_oracle or quickvina reward_vina
        #raw_docking_scores, docking_rewards = self.chemprop_oracle(smiles) #adjust checkpoints for specific targets
        #raw_docking_scores, docking_rewards = self.gradient_boosting_oracle(smiles)
        qed_rewards = reward_qed(mols)
        raw_sa, sa_rewards = reward_sa(mols)

        # Formula used in GEAM paper
        aggregated_rewards = (np.clip(docking_rewards, 0, 20) / 20) * (qed_rewards) * (sa_rewards)

        return raw_docking_scores, qed_rewards, raw_sa, aggregated_rewards
